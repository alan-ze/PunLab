...
def get_target_directory():
    ...
    if len(sys.argv) > 1:
        ...
        path = input("Please paste the full directory path containing the CSV files:\n> ")
...
def process_directory(target_dir):
    ...
    for group_id, files in file_groups.items():
        ...
        print(f"Processing group '{group_id}' -> Creating '{os.path.basename(output_filename)}'...")
        ...
        for csv_file in files:
            try:
                ...
                # Verify columns
                if 'Angular frequency' not in df.columns or 'Oscillation strain' not in df.columns:
